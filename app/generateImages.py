...
def startGenerateImages(metadata, imageUrlsMap, projectLayersDepth, orderId):
    ...
    sortedLayerNames = sorted(layerNames, key=lambda layerName: projectLayersDepth[layerName] , reverse = True)
    for metadataBlock in metadata:
        image = imageio.imread(getImagesPath(metadataBlock[sortedLayerNames[0]] + ".png",orderId))
